...
export class PopupRenderer {
  static createWordItemHTML(word: HighlightWord, realIndex: number, isSelected: boolean, listBackground: string, listForeground: string): string {
    ...
    const menuTitle = browserAPI.i18n.getMessage('word_actions') || 'Actions';
    ...
    return `
      <div class="word-item ${isSelected ? 'selected' : ''}" data-index="${realIndex}">
        ...
        <div class="word-item-actions">
          <button class="word-item-icon-btn edit-word-btn" data-index="${realIndex}" title="${browserAPI.i18n.getMessage('edit') || 'Edit'}">
            <i class="fa-solid fa-pen"></i>
          </button>
          <input type="color" value="${bgColor}" data-bg-edit="${realIndex}" class="word-item-color-picker" title="${browserAPI.i18n.getMessage('background_color_title') || 'Background color'}">
          <input type="color" value="${fgColor}" data-fg-edit="${realIndex}" class="word-item-color-picker" title="${browserAPI.i18n.getMessage('text_color_title') || 'Text color'}">
          <label class="word-item-eye-toggle" title="${browserAPI.i18n.getMessage('toggle_active') || 'Toggle active'}" aria-label="${browserAPI.i18n.getMessage('toggle_active') || 'Toggle active'}">
      ...
      <div class="page-highlight-item" data-word="${DOMUtils.escapeHtml(highlight.word)}" style="border-left-color: ${highlight.background}; --item-tint: ${highlight.background};">
        ...
        ${highlight.count > 1 ? `
          <div class="page-highlight-nav">
            <button class="highlight-prev" title="${browserAPI.i18n.getMessage('previous') || 'Previous'}">
              <i class="fa-solid fa-chevron-up"></i>
            </button>
            <button class="highlight-next" title="${browserAPI.i18n.getMessage('next') || 'Next'}">
  ...
  static createExceptionItemHTML(domain: string): string {
    return `
      <div class="exception-item">
        ...
        <button type="button" class="exception-remove" data-domain="${DOMUtils.escapeHtml(domain)}" title="${DOMUtils.escapeHtml(browserAPI.i18n.getMessage('remove') || 'Remove')}" aria-label="${DOMUtils.escapeHtml(browserAPI.i18n.getMessage('remove') || 'Remove')}">
  ...
  static createPaginationHTML(currentPage: number, totalPages: number, totalWords: number, pageSize: number): string {
    ...
    const showingText = browserAPI.i18n.getMessage('showing_items')
      ?.replace('{start}', String(startItem))
      .replace('{end}', String(endItem))
      .replace('{total}', String(totalWords))
      || `Showing ${startItem}-${endItem} of ${totalWords} words`;

    const pageInfoText = browserAPI.i18n.getMessage('page_info')
      ?.replace('{current}', String(currentPage))
      .replace('{total}', String(totalPages))
      || `Page ${currentPage} of ${totalPages}`;

    const firstPageTitle = browserAPI.i18n.getMessage('first_page') || 'First page';
    const prevPageTitle = browserAPI.i18n.getMessage('previous_page') || 'Previous page';
    const nextPageTitle = browserAPI.i18n.getMessage('next_page') || 'Next page';
    const lastPageTitle = browserAPI.i18n.getMessage('last_page') || 'Last page';
